 ...
 * We have inherited MAX_ZONES limit from Illumos.
 */
...
//?	char		*zone_rootpath;	/* Path to zone's root + '/' */
	...
	struct user_namespace *zone_user_ns;
	list_t		zone_cached_user_ns;
	struct task_struct *start_pid;
} zone_t;

typedef struct zone_userns_cache {
	list_node_t	zuc_node;
	zone_t		*zuc_zone;
} zone_userns_cache_t;
